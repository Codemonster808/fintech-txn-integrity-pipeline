...
the gate binary built (cd src/gate && go build ./...).
"""
import ast
import json
import subprocess
...
GATE_BIN = REPO_ROOT / "src" / "gate" / "gate"
N_EVENTS = 1000
RETRY_RATE = 0.08
...
def gate_process():
    if not GATE_BIN.exists():
        pytest.skip(f"gate binary not built — run `cd src/gate && go build ./...` first ({GATE_BIN})")
...
def test_full_pipeline_quality(gate_process):
    ...
    gen = subprocess.run(
        [sys.executable, "src/data_gen.py", "--out", str(data_path),
         "--count", str(N_EVENTS), "--retry-rate", str(RETRY_RATE), "--seed", "123"],
    ...
    consume = subprocess.run(
        [sys.executable, "src/consumer.py", "--idle-timeout", "8"],
    ...
    t0 = time.perf_counter()
    sm = subprocess.run(
        [sys.executable, "src/statemachine.py"],
    ...
    curate2 = subprocess.run(
        [sys.executable, "src/curate.py"], cwd=REPO_ROOT, capture_output=True, text=True, timeout=120,
    )
    ...
    report.check(
        Dimension.COMPLETENESS, "no_settled_txn_lost",
        measured=n_raw_valid, threshold=len(unique_keys),
        detail=f"S3 raw valid objects ({n_raw_valid}) vs unique idempotency keys generated ({len(unique_keys)})",
    )

    measured_dup_rate = consume_stats["duplicate_rejected"] / (consume_stats["processed"] + consume_stats["duplicate_rejected"])
    rate_delta = abs(measured_dup_rate - RETRY_RATE)
    report.check(
        Dimension.CORRECTNESS, "dedup_rate_matches_injected", measured=round(rate_delta, 4),
        threshold=0.02, higher_is_better=False,
        detail=f"measured dedup rate {measured_dup_rate:.4f} vs injected {RETRY_RATE}",
    )
    report.check(
        Dimension.CORRECTNESS, "zero_processing_errors", measured=consume_stats["errors"], threshold=0,
        higher_is_better=False, detail="consumer error count",
    )
    ...
    report.check(
        Dimension.CONSISTENCY, "curate_reprocess_same_row_count",
    ...
    report.check(
        Dimension.VALIDITY, "no_valid_events_misrouted_to_quarantine",
        measured=n_quarantine, threshold=0, higher_is_better=False,
        detail="all synthetic events use schema_version=1, so quarantine should be empty",
    )

    report.check(
        Dimension.TIMELINESS, "daily_job_under_sla", measured=round(daily_job_seconds, 1),
        threshold=120.0, higher_is_better=False, detail="preflight + curate + postflight wall time",
    )
    report.check(
        ...
        Dimension.TIMELINESS, "consumer_throughput_under_sla", measured=round(consume_seconds, 1),
        threshold=45.0, higher_is_better=False, detail=f"{N_EVENTS} events through gate+validator",
    )
