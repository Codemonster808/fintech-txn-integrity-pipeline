...
def test_concurrent_duplicate_requests_only_one_wins(gate_process):
    ...
    assert status_codes.count(200) == 1, (
        f"expected exactly 1 acceptance, got {status_codes.count(200)}"
    )
...
def test_burst_of_distinct_keys_all_succeed(gate_process):
    ...
    def send(i: int):
        return requests.post(
            f"{GATE_URL}/accept", json=_event(f"idem-burst-{run_id}-{i}", f"txn-burst-{i}")
        )
...
def test_accept_batch_dedupes_within_and_across_requests(gate_process):
    ...
    statuses_for_new_key = [
        r["status"] for r in resp.json()["results"] if r["idempotency_key"] == new_key
    ]
...
def test_accept_batch_race_window_is_real_not_hypothetical(gate_process):
    ...
    src/ingestion/gate/main.go as introducing a race window /accept does not have:
