...
(src/outbox_publisher.py). A duplicate row here is recoverable; the
...
Usage:
    python3 src/curate_incremental.py --n-batches 10 --batch-rows 2000000
"""
...
def _make_partition_filter(endpoint: str, region: str, access_key: str, secret_key: str, bloom_snapshot: BloomFilter):
    ...
    def _filter_partition(rows_iter):
        ...
        for i in range(0, len(maybe_keys), 100):
            ...
            resp = ddb.batch_get_item(
                RequestItems={CURATED_KEYS_TABLE: {"Keys": [{"idempotency_key": {"S": k}} for k in chunk]}}
            )
            for item in resp.get("Responses", {}).get(CURATED_KEYS_TABLE, []):
                existing_keys.add(item["idempotency_key"]["S"])

        new_rows = definitely_new + [r for r in maybe_seen if r["idempotency_key"] not in existing_keys]
...
def process_batch(spark, batch_id: int, n_rows: int, bloom: BloomFilter, cross_batch_sample: list[str]) -> dict:
    from pyspark.sql import functions as F
    from pyspark.sql.window import Window

    from scale_bench import synthetic_events_df  # reuses the verified ~8% intra-batch collision generator
    ...
    df = df.withColumn("idempotency_key", F.concat(F.lit(f"b{batch_id}-"), F.col("idempotency_key")))
    ...
    if n_replays:
        ...
        replay_ts = datetime(2026, 1, 1, tzinfo=timezone.utc)
        replay_rows = [
            Row(idempotency_key=k, txn_id=f"replay-{batch_id}-{i}", account_id="acc_replay",
                amount_cents=100, currency="USD", schema_version=1, ts=replay_ts)
