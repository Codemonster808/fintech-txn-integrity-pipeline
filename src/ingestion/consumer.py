...
from models.validator import handler as validate_event  # noqa: E402
...
def process_message(body: str) -> dict:
    ...
    if gate_resp.status_code != 200:
        raise RuntimeError(
            f"gate returned unexpected status {gate_resp.status_code}: {gate_resp.text}"
        )
...
def run(max_messages: int | None = None, idle_timeout_s: float = 5.0) -> dict:
    ...
    while True:
        if (
            ...
            and stats["processed"] + stats["duplicate_rejected"] >= max_messages
        ):
            break
