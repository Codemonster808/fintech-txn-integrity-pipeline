...
def deploy_state_machines(sfn) -> dict:
    arns = {}
    existing = {sm["name"]: sm["stateMachineArn"] for sm in sfn.list_state_machines()["stateMachines"]}
...
def run_daily_job() -> dict:
    ...
    curate_proc = subprocess.run(
        [sys.executable, str(Path(__file__).resolve().parent / "curate.py")],
    ...
    stats_line = next((l for l in curate_proc.stdout.splitlines() if l.startswith("curated: ")), None)
    stats = ast.literal_eval(stats_line[len("curated: "):]) if stats_line else {}

    print("Running post-flight...")
    postflight_result = run_execution(sfn, arns["txn-daily-postflight"], {
        "job_id": job_id,
    ...
    return {
        "job_id": job_id,
        "status": "completed" if postflight_result["status"] == "SUCCEEDED" else "postflight_failed",
        "curate_stats": stats,
    }
