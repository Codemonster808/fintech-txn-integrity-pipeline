...
shuffle as src/curate.py), plus a "logical" mode that processes billions
...
Usage:
    python3 src/scale_bench.py --mode materialized --scales 10000,100000,1000000,10000000
    python3 src/scale_bench.py --mode logical --rows 2000000000
"""
...
def synthetic_events_df(spark, n: int):
    ...
    df = df.withColumn(
        ...
        F.when(
            ...
            F.concat(F.lit("orig-"), F.floor(F.rand(seed=99) * F.greatest(F.col("row_id"), F.lit(1))).cast("string")),
        ).otherwise(F.concat(F.lit("orig-"), F.col("row_id").cast("string"))),
    )
    df = df.withColumn("txn_id", F.sha2(F.concat(F.lit("txn-"), F.col("row_id").cast("string"), F.rand(seed=7).cast("string")), 256))
    df = df.withColumn("account_id", F.concat(F.lit("acc_"), (F.col("row_id") % 2000).cast("string")))
...
def _max_completed_stage_id(spark) -> int:
    ...
    if not base:
        return -1
    try:
        resp = requests.get(f"{base}/api/v1/applications/{_app_id(spark)}/stages?status=complete", timeout=5)
        stages = resp.json()
...
def run_materialized_scale(spark, n: int) -> dict:
    ...
    return {
        ...
        "rows_per_second_dedup_phase": round(n / dedup_and_write_s, 1) if dedup_and_write_s > 0 else None,
...
def run_logical(spark, n: int) -> dict:
    ...
    result = (
        ...
        .agg(F.sum("n").alias("total_rows"), F.sum("total_amount_cents").alias("total_amount_cents"))
        .collect()[0]
    )
...
def extrapolate_to_1tb(materialized_results: list[dict]) -> dict:
    ok = [r for r in materialized_results if r.get("status") == "ok" and r.get("rows_per_second_dedup_phase")]
    if not ok:
...
def write_report(materialized_results: list[dict], logical_result: dict | None, extrapolation: dict | None,
    ...
    if materialized_results:
        lines += ["", "## Materialized curve (real Parquet, real shuffle)", "",
    ...
    for r in materialized_results:
        if r.get("status") == "ok":
            ...
            lines.append(f"| {r['scale_rows']:,} | **FAILED** | — | — | — | {r.get('reason', '')} |")

    if logical_result:
        lines += ["", "## Logical mode (in-flight, nothing written to disk)", "",
    ...
    if extrapolation and "error" not in extrapolation:
        ...
        if gate:
            lines += ["", "### The real bottleneck isn't Spark", "",
...
def main() -> None:
    ...
    parser.add_argument("--scales", default="10000,100000,1000000,10000000",
                         help="comma-separated row counts, materialized mode only")
    parser.add_argument("--rows", type=int, default=2_000_000_000, help="row count, logical mode only")
    ...
    try:
        if args.mode == "materialized":
            ...
            write_report([], logical, None, args.json_out.replace(".json", "-logical.json"),
