...
REQUIRED_FIELDS = {"txn_id", "idempotency_key", "account_id", "amount_cents", "currency", "schema_version", "ts"}
...
def validate_event(event: dict, expected_version: int) -> tuple[bool, str]:
    ...
    if event.get("schema_version") != expected_version:
        return False, f"schema_version mismatch: got {event.get('schema_version')}, expected {expected_version}"
